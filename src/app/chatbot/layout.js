...
import {handleNewChat} from '@/app/reduxToolkit/chatSlice'
...
{children}
  ...
  <div className="drawer-side">
    ...
    <ul className="menu  text-white h-full w-70 p-4 bg-[#1D1E22]">
      ...
      <li className='hover:bg-[#2F2F39] rounded-sm hover:cursor-pointer'><Link href={'/chatbot'} onClick={()=> { console.log('Button clicked'); dispatch(handleNewChat(chatHistory))}}>
...
New Chat</Link>
      ...
      <li className='hover:bg-[#2F2F39]  rounded-sm hover:cursor-pointer'> <Link href={'/chatbot'}>Chat History</Link>

      {chatHistory ? chatHistory.map((item)=>( <ul>
          <li key={item.id}><Link href={'/chatbot'}>{item.id}</Link></li>
        </ul>)):('no chat found')}
